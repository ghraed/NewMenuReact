...
import { confirmPendingOrder, fetchPendingOrders } from '../services/orderService';
import type { ConfirmOrderRequest, DiscountType, OrderRecord } from '../types';
...
const StaffOrdersPage: React.FC = () => {
  ...
  const handleConfirm = async (order: OrderRecord) => {
    ...
    try {
      const response = await confirmPendingOrder(order.id, payload);
      setOrders((current) => current.filter((item) => item.id !== order.id));
      setNotice(`Confirmed ${response.order.order_number || `order #${response.order.id}`}.`);
    } catch (err: unknown) {
  ...
  return (
    <DashboardLayout title="Pending Orders">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-semibold text-text">
            {user?.role === 'staff' ? 'Orders waiting for your confirmation' : 'Restaurant order confirmations'}
          </h2>
      ...
      {loading ? (
        <div className="py-12 text-center text-muted">Loading pending orders...</div>
      ) : null}

      {!loading && orders.length === 0 ? (
        <div className="py-12 text-center">
          <div className="mb-4 text-5xl">🧾</div>
          <h3 className="mb-2 text-xl font-medium text-text">No pending confirmations</h3>
          <p className="text-muted">New guest order requests will appear here when they are created.</p>
        </div>
      ) : null}
            ...
            return (
              <GlassCard key={order.id} className="space-y-5">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <p className="text-xs uppercase tracking-[0.18em] text-gold2/85">Pending Confirmation</p>
                  ...
                  <div className="rounded-2xl border border-gold/20 bg-gold/10 px-4 py-3 text-right">
                    <p className="text-xs uppercase tracking-[0.18em] text-gold2/85">Guest Total</p>
                ...
                <div className="grid gap-4 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
                  ...
                  <div className="rounded-[26px] border border-white/10 bg-white/[0.03] p-4">
                    <p className="text-sm font-semibold text-text">Confirmation Settings</p>
                    <div className="mt-4 grid gap-3">
                      ...
                      <div className="rounded-[22px] border border-white/10 bg-black/10 p-4">
                        <div className="flex items-center justify-between gap-3 text-sm text-muted">
                          ...
                          <span className="font-medium text-text">${order.invoice.subtotal}</span>
                        </div>
                        <p className="mt-3 text-xs text-muted2">
                          The final invoice is recalculated by the API when you confirm this order.
                        </p>
                      </div>
